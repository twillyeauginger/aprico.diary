import { authenticatedUser, userScopedClient } from "../_shared/auth.ts";
import {
  handlePreflight,
  json,
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { photoPath } = (await request.json()) as { photoPath?: string };
    if (!photoPath || !photoPath.startsWith(`${user.id}/`)) {
      return json(request, { error: "사진 경로가 올바르지 않습니다." }, 400);
    }
    ...
    if (!openAiKey) {
      return json(
        request,
        { error: "OpenAI API 키가 아직 Edge Function에 연결되지 않았습니다." },
        503,
      );
    }
    ...
    if (downloadError || !photo) {
      return json(request, { error: "업로드한 사진을 찾지 못했습니다." }, 404);
    }
    if (!allowedTypes.has(photo.type)) {
      return json(request, { error: "지원하지 않는 사진 형식입니다." }, 415);
    }
    if (photo.size > 8 * 1024 * 1024) {
      return json(request, { error: "사진은 8MB 이하로 올려주세요." }, 413);
    }
